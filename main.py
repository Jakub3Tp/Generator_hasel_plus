    ...
    def generate_password(self):
        ...
        try:
            ...
            if self.ui.letter_size_check.isChecked():
                rand = random.randint(0, password_length - 1)
                password[rand] = random.choice(big_letters)

            if self.ui.number_check.isChecked():
                rand = random.randint(0, password_length - 1)
                while password[rand] not in small_letters:
                    rand = random.randint(0, password_length - 1)
                password[rand] = random.choice(numbers)

            if self.ui.special_check.isChecked():
                rand = random.randint(0, password_length - 1)
                while password[rand] not in small_letters:
                    rand = random.randint(0, password_length - 1)
                password[rand] = random.choice(special_character)
